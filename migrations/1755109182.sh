...
# Users can still opt-in to Cloudflare DNS using: omarchx-setup-dns cloudflare
...
if [[ -f /etc/systemd/resolved.conf ]]; then
  ...
  if [[ -f /etc/systemd/network/99-omarchx-dns.network ]]; then
    sudo rm -f /etc/systemd/network/99-omarchx-dns.network
  ...
  echo "To use Cloudflare DNS, run: omarchx-setup-dns Cloudflare"
fi