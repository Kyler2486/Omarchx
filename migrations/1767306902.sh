...
if [[ -d $OMARCHX_DIR/themes ]]; then
  ...
  # Get list of git-tracked background files (relative to omarchx dir)
...
THEMES_DIR="$HOME/.config/omarchx/themes"
CURRENT_THEME_LINK="$HOME/.config/omarchx/current/theme"
...
elif [[ -f $HOME/.config/omarchx/current/theme.name ]]; then
  CURRENT_THEME_NAME=$(cat "$HOME/.config/omarchx/current/theme.name")
fi

# Remove all symlinks from ~/.config/omarchx/themes
...
if [[ -n $CURRENT_THEME_NAME ]]; then
  omarchx-theme-set "$CURRENT_THEME_NAME"
else
  # Backup to ensure a theme is set if we can't deduce the name
  omarchx-theme-set "Tokyo Night"
fi
