echo "Migrate Hyprland user config from Omarchx hyprlang files to Lua files"

hypr_config_dir="$HOME/.config/hypr"
omarchx_hypr_config_dir="$OMARCHX_PATH/config/hypr"
toggle_state_dir="$HOME/.local/state/omarchx/toggles/hypr"
toggle_defaults_dir="$OMARCHX_PATH/default/hypr/toggles"
timestamp=$(date +%s)
...
for lua_config in "$omarchx_hypr_config_dir"/*.lua "$omarchx_hypr_config_dir"/.luarc.json; do
...
if [[ -f $sddm_hypr_conf || -f $sddm_wayland_conf ]]; then
  if [[ -f $OMARCHX_PATH/default/sddm/hyprland.lua ]]; then
    sudo mkdir -p /usr/share/sddm
    sudo cp "$OMARCHX_PATH/default/sddm/hyprland.lua" "$sddm_hypr_lua"
  fi
