...
OBSIDIAN_DEFAULT_FLAGS_FILE="$OMARCHX_PATH/config/obsidian/user-flags.conf"
...
if [[ ! -f $OBSIDIAN_FLAGS_FILE ]]; then
  ...
  if [[ -n $missing_flags ]]; then
    {
      echo
      echo "# Added by Omarchx migration 1776434586: Obsidian launch flags moved from Hyprland binding."
      printf "%s\n" "$missing_flags"
    } >>"$OBSIDIAN_FLAGS_FILE"
  fi
fi
