--
-- Dynamic Omarchx Unlocks Menu for Elephant/Walker
--
-- A "Default" entry restores the omarchx-shipped Plymouth via
-- omarchx-plymouth-reset. After that, every theme that has a preview-unlock.png
-- appears as a customised unlock; picking one runs omarchx-plymouth-set-by-theme
-- <theme>. Both run in a floating terminal so sudo can prompt.
--
Name = "omarchxunlocks"
NamePretty = "Omarchx Unlocks"
...
function GetEntries()
  ...
  local user_themes_dir = home .. "/.config/omarchx/themes"
  local omarchx_path = os.getenv("OMARCHX_PATH") or ""
  local default_themes_dir = omarchx_path .. "/themes"
  local default_preview = omarchx_path .. "/default/plymouth/preview-unlock.png"
      ...
      if theme_name and not seen_themes[theme_name] then
        ...
        if file_exists(preview_path) then
          ...
          table.insert(entries, {
            ...
            PreviewType = "file",
            Actions = {
              activate = "omarchx-launch-floating-terminal-with-presentation "
                .. shell_escape("omarchx-plymouth-set-by-theme " .. shell_escape(theme_name)),
            },
          })
        end
      end
  ...
  local default_entry = {
    ...
    Actions = {
      activate = "omarchx-launch-floating-terminal-with-presentation "
        .. shell_escape("omarchx-plymouth-reset"),
    },
  }
