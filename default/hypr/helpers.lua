...
local function command_from(value, description)
  ...
  if value.omarchx then
    return "omarchx-launch-" .. value.omarchx
  ...
  elseif value.tui then
    if value.focus then
      return "omarchx-launch-or-focus-tui " .. shell_quote(value.tui)
    else
      return "omarchx-launch-tui " .. shell_quote(value.tui)
    end
  end
...
function o.launch_webapp(url)
  return "omarchx-launch-webapp " .. shell_quote(url)
end

function o.launch_webapp_sole(name, url)
  return "omarchx-launch-or-focus-webapp " .. shell_quote(name) .. " " .. shell_quote(url)
end

function o.launch_sole(match, command)
  return "omarchx-launch-or-focus " .. shell_quote(match) .. " " .. shell_quote(o.launch(command))
end

function o.bind_menu(keys, description, menu, options)
  o.bind(keys, description, menu and ("omarchx-menu " .. menu) or "omarchx-menu", options)
end

function o.bind_toggle(keys, description, toggle, options)
  o.bind(keys, description, "omarchx-toggle-" .. toggle, options)
end
